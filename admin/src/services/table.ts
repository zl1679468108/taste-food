...
export const listTables = (shopId: string) =>
  request.get(`/api/merchant/shops/${shopId}/tables`) as Promise<ShopTable[]>;

export const createTable = (
  shopId: string,
  data: { tableNo: string; label?: string; sortOrder?: number; active?: boolean },
) => request.post(`/api/merchant/shops/${shopId}/tables`, data) as Promise<ShopTable>;

export const updateTable = (
  shopId: string,
  tableId: string,
  data: Partial<{ tableNo: string; label: string; sortOrder: number; active: boolean }>,
) => request.patch(`/api/merchant/shops/${shopId}/tables/${tableId}`, data) as Promise<ShopTable>;

export const deleteTable = (shopId: string, tableId: string) =>
  request.delete(`/api/merchant/shops/${shopId}/tables/${tableId}`);

export const seedTables = (shopId: string) =>
  request.post(`/api/merchant/shops/${shopId}/tables/seed`) as Promise<ShopTable[]>;
