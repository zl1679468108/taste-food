...
（无 — 当前 PRD 范围内无未启动的代码任务；迁移落地见「暂缓/跟进」M-001）
...
| T313.7 | 顾客标签后端（CRUD + 分配） | 2026-08-03 | customer.service/controller 新增；tf_customer_tags + tf_customer_tag_relations（v32）；列表支持 tagIds 过滤并返回 tags；同名冲突 409 |
| T313.8 | 顾客标签前端（列/筛选/管理/分配） | 2026-08-03 | 列表标签列 + 按标签多选筛选 + TagManageModal（店铺标签 CRUD）+ 抽屉 TagAssignModal（勾选分配 + 快速新建） |
...
| M-001 | 将待执行迁移 apply 到生产 Supabase | 2026-08-04 | 经 PAT + Management API `/v1/projects/{ref}/database/query` 落地 v29(voice_alert_config)/v30(get_today_stats RPC)/v31(tf_users.status)/v32(tf_customer_tags+tf_customer_tag_relations+tf_messages)；已验证对象落库；顾客管理/标签/站内信可跑真实数据 |
